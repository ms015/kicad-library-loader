...
def launch(settings):
    ...
    ttk.Label(box, text='CSE / UltraLibrarian ZIP → KiCad 10    •    LCSC → easyeda2kicad').pack(anchor='w', pady=(0, 12))
    ...
    def choose_zip():
        paths = filedialog.askopenfilenames(title='CSE / UltraLibrarian ZIP', filetypes=[('ZIP', '*.zip')])
        for path in paths:
            jobs.put(('zip', path))
